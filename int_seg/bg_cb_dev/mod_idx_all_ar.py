...
#                                       subj_lst)
#     np.save(f'subjs_efc_mat_{task}_v2.npy', efc_mat_allsub)


# # get efc mat for sing subj at a time
# task = 'stroop'
# for subj in subj_lst:
#     efc_mat_sub = jr.get_efc_trans_sing(opj(main_dir, d_path), task, subj)
#     print(efc_mat_sub)
...
task = 'stroop' # stroop
# efc_mat_allsub = jr.get_efc_trans(opj(main_dir, d_path), task, \
#                                   subj_lst[0:math.floor(len(subj_lst)*0.70)]) # 75% data
# np.save(f'subjs_efc_mat_{task}_v2.npy', efc_mat_allsub)
# sys.exit()


# load saved npy with task coalesce matrices
mat_stroop_all = np.load('subjs_181_net_all/subjs_181_efc_mat_stroop.npy')
...
mod_max_stroop_all = np.load('subjs_181_net_all/subjs_181_mod_max_stroop.npy', allow_pickle=True)
...
plt.tight_layout()
# plt.savefig('allsub_mod_qall_smooth_sig2p5_blocks_mask_init.png', dpi=300)  # use this
plt.show()
##############
...
mod_max_rest_all = np.load('subjs_181_net_all/subjs_181_mod_max_rest.npy', allow_pickle=True)
...
ar_model = AutoReg(df_q_all_reg['mod_idx'], exog=df_q_all_reg[['inc_reg', 'con_reg']], lags=1).fit() # frs
...
######## matrix visualization - incongruent - # try this with the beta coeffs
