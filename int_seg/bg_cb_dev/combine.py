...
plt.xlabel('Lags', size=15, fontname='serif', weight='bold')
fig.supylabel('Correlation', size=15, fontname='serif', weight='bold')
plt.tight_layout()
plt.savefig('cross_corr_eigenvec_cen_bg_cb_mod_cort_subplots_avg_noerr.png', dpi=300)
plt.show()
# plt.close()
...
# lags = signal.correlation_lags(len(eigen_bg_avg_blocks), len(eigen_cb_avg_blocks))
lags = cross_corr_vec(eigen_cb_avg_blocks, eigen_bg_avg_blocks, lags=20)[1]
...
#                         zip(q_allsub_avg_blocks_smooth_z, eigen_cb_allsub_avg_blocks_smooth_z))))
cross_corr_modxcb = np.array(list(map(lambda sub: cross_corr_vec(sub[0], sub[1], lags=20)[0], \
                        zip(q_allsub_avg_blocks_smooth_z, eigen_cb_allsub_avg_blocks_smooth_z))))
...
cross_corr_modxbg = np.array(list(map(lambda sub: cross_corr_vec(sub[0], sub[1], lags=20)[0], \
                        zip(q_allsub_avg_blocks_smooth_z, eigen_bg_allsub_avg_blocks_smooth_z))))
...
cross_corr_cbxbg = np.array(list(map(lambda sub: cross_corr_vec(sub[0], sub[1], lags=20)[0], \
                        zip(eigen_cb_allsub_avg_blocks_smooth_z, eigen_bg_allsub_avg_blocks_smooth_z))))
...
ax[0].fill_between(lags, cross_corr_modxcb_avg-cross_corr_modxcb_std, \
    cross_corr_modxcb_avg+cross_corr_modxcb_std, lw=0, color=p_dict['var_corr_cort_cb'], alpha=0.3)
...
ax[1].fill_between(lags, cross_corr_modxbg_avg-cross_corr_modxbg_std, \
    cross_corr_modxbg_avg+cross_corr_modxbg_std, lw=0, color=p_dict['var_corr_cort_bg'], alpha=0.3)
...
ax[2].fill_between(lags, cross_corr_cbxbg_avg-cross_corr_cbxbg_std, \
    cross_corr_cbxbg_avg+cross_corr_cbxbg_std, lw=0, color=p_dict['var_corr_cb_bg'], alpha=0.3)
...
for i in range(1,36):
    ...
    var_stderr_lags_lst.append(impy_vars[1])
...
np.save(f'var_stderr_lags35.npy', var_stderr_lags_lst)
np.save(f'var_corr_lags35.npy', corr_lags_lst)
np.save(f'var_corr_std_lags35.npy', std_lags_lst)
np.save(f'var_corr_sem_lags35.npy', stderr_lags_lst)
"""
...
var_corr_sem_lags35 = np.load('var_corr_sem_lags35.npy')
...
# PLOT lag35 VAR coefficients and correlation - subplots - OHBM version
...
# ax2.fill_between(list(range(1, len(var_corr_lags35.T[0])+1)), var_corr_lags35.T[0]-var_corr_std_lags35.T[0]/math.sqrt(242), \
#     var_corr_lags35.T[0]+var_corr_std_lags35.T[0]/math.sqrt(242), lw=0, color=p_dict['var_corr_cort_cb'], alpha=0.3)
...
# ax2.fill_between(list(range(1, len(var_corr_lags35.T[1])+1)), var_corr_lags35.T[1]-var_corr_std_lags35.T[1]/math.sqrt(242), \
#     var_corr_lags35.T[1]+var_corr_std_lags35.T[1]/math.sqrt(242), lw=0, color=p_dict['var_corr_cort_bg'], alpha=0.3)
...
# ax2.fill_between(list(range(1, len(var_corr_lags35.T[2])+1)), var_corr_lags35.T[2]-var_corr_std_lags35.T[2]/math.sqrt(242), \
#     var_corr_lags35.T[2]+var_corr_std_lags35.T[2]/math.sqrt(242), lw=0, color=p_dict['var_corr_cb_bg'], alpha=0.3)
...
# plot inset
left, bottom, width, height = [0.688, 0.744, 0.2, 0.2] # x, y
...
plt.tight_layout()
plt.savefig('var_coeffs_corr_eigenvec_cen_bg_cb_mod_cort_subplots_yerr_inset.png', dpi=2000)  # 2000
plt.show()
