...
ax.plot(np.arange(group_blocks[0][0]*rt, (group_blocks[0][1]+1)*rt, 2), gaussian_filter(mod_cort_avg_blocks, sigma=1), lw=1, c=p_dict['cort_line_cb'], ls='--')
...
ax2 = ax.twinx()
ax2.plot(np.arange(group_blocks[0][0]*rt, (group_blocks[0][1]+1)*rt, 2), gaussian_filter(eigen_bg_avg_blocks, sigma=0.8), lw=1, c=p_dict['bg_line_eig']) # 0.5
...
ax2.plot(np.arange(group_blocks[0][0]*rt, (group_blocks[0][1]+1)*rt, 2), gaussian_filter(eigen_cb_avg_blocks, sigma=0.8), lw=1, c=p_dict['cb_line_eig']) # 0.5
ax2.fill_between(np.arange(group_blocks[0][0]*rt, (group_blocks[0][1]+1)*rt, 2), \
...
fig.suptitle(task.upper(), size=20, fontname='serif', fontweight='bold', x=0.575)

# mod x cb
ax[0].axhline(y=0, c='k', lw=1.2, alpha=0.28, ls='--', dashes=(11, 21))
ax[0].plot(lags*rt, cross_corr_modxcb_avg, lw=1.7, c=p_dict['var_corr_cort_cb'])
ax[0].fill_between(lags*rt, cross_corr_modxcb_avg-cross_corr_modxcb_ci, \
    cross_corr_modxcb_avg+cross_corr_modxcb_ci, lw=0, color=p_dict['var_corr_cort_cb'], alpha=0.3)
ax[0].set_title('Cb \u2192 Cortex', size=12, fontname='serif', weight='bold')
ax[0].set_ylim(-0.1, 0.25)
ax[1].set_yticks(np.arange(-0.1, 0.2, 0.1))

# mod x bg
ax[1].axhline(y=0, c='k', lw=1.2, alpha=0.28, ls='--', dashes=(11, 21))
ax[1].plot(lags*rt, cross_corr_modxbg_avg, lw=1.7, c=p_dict['var_corr_cort_bg'])
ax[1].fill_between(lags*rt, cross_corr_modxbg_avg-cross_corr_modxbg_ci, \
    cross_corr_modxbg_avg+cross_corr_modxbg_ci, lw=0, color=p_dict['var_corr_cort_bg'], alpha=0.3)
ax[1].set_title('Bg \u2192 Cortex', size=12, fontname='serif', weight='bold')
ax[1].set_ylim(-0.1, 0.1)
ax[1].set_yticks(np.arange(-0.1, 0.15, 0.1))
...
ax[2].set_title('Bg \u2192 Cb', size=12, fontname='serif', weight='bold')
ax[2].set_ylim(-0.15, 0.1)
ax[2].set_yticks(np.arange(-0.15, 0.1, 0.1))

ax[0].text(-40.69, 0.25, 'A', size=30, fontname='serif')
ax[1].text(-40.69, 0.1, 'B', size=30, fontname='serif')
ax[2].text(-40.69, 0.1, 'C', size=30, fontname='serif')
...
fig.suptitle(task.upper(), size=20, fontname='serif', fontweight='bold', x=0.53) # , x=0.575
...
ax1.set_ylabel('F-statistic', size=11, fontname='serif')
ax1.set_xlabel('Lags', size=11, fontname='serif')
...
ax2.set_xlabel('Lags', size=11, fontname='serif')
ax2.legend(prop={'size': 7.5}, loc=1)

# fig.supxlabel('Lags', size=11, fontname='serif')
plt.tight_layout()
...
ax1.set_title(task.upper(), size=20, fontname='serif', fontweight='bold')
...
plt.suptitle(task.upper(), size=30, fontname='serif', fontweight='bold')
