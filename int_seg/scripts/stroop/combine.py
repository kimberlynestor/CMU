...
ax.plot(np.arange(group_blocks[0][0]*rt, (group_blocks[0][1]+1)*rt, 2), gaussian_filter(mod_cort_avg_blocks, sigma=1), lw=3, c=p_dict['cort_line_cb'], ls=':')
...
plt.tight_layout()
plt.savefig(f'{pars[1]}/output/{task}/eigenvec_cen_bg_cb_mod_cort_smooth_sig2_avg_blocks_cb_z_yerr_std_{task}.png', dpi=2000)
plt.show()
...
plt.suptitle(task.upper(), size=30, fontname='serif')
